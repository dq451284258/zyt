<template>
<div class="login-forgetpassword-m">
      ...
      <div class="main-body-content">
        <el-form :model="registForm" ref="registForm" label-width="0" :rules="rules" >
          ...
          <el-form-item label="" prop="code" :rules="[{ required: true, message: '验证码不能为空'}]">
            <el-input class="code" type="text" maxlength="6" placeholder="请输入验证码" v-model="registForm.code"></el-input><el-button class="code-btn" type="primary" @click="codeButton.click &&  postCode()">{{ codeButton.value }}</el-button>
          </el-form-item>
          ...
          <el-form-item label="" class="btn">
            <el-button type="primary" @click="submitForm('registForm')" class="btn-regist">提交</el-button>
          </el-form-item>
        </el-form>
      </div>
    </div>
...
export default {
  ...
  data() {
    var validatePass = (rule, value, callback) => {
      let passREG = /^(?![^a-zA-Z]+$)(?!\D+$)/;
      if (value === '') {
        callback(new Error('请输入密码'));
      } else {
        if(!passREG.test(this.registForm.password || this.registForm.password.length < 6)) {
          callback(new Error('密码需字母数字英文符号，且至少6位数!'));
        }
  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$ajax.post('/user/forget',{
            mobile: this.registForm.mobile,
            code: this.registForm.code,
            password: this.registForm.password
          }).then((res) => {
            if(res.data.code === 1) {
              Toast({ message: '恭喜你，修改密码成功' });
              setTimeout(() => {
                this.$router.push('/m/login')
              },1000)
            }else {
              Toast('修改失败' + res.data.message);
            }
          }).catch((error) => {
            if (error.response) {
              Toast('修改失败' + error.response.data.message);
            }
          })
    ...
    postCode() {
      this.$refs['registForm'].validateField('mobile',(valid) => {
        if(valid === '') {
          this.$ajax.post('/user/sendcode',{ mobile:this.registForm.mobile, smsType: 1 })
          .then((res) => {
            if(res.data.code === 1) {
